#!/usr/bin/env node
// netlify/migrate.js — runs during Netlify build
...
const TABLES = [
  ...
  `CREATE TABLE IF NOT EXISTS t365_webhook_events (
...
async function run() {
  ...
  for (const sql of TABLES) {
    ...
    try {
      ...
      if (r.ok) { console.log(`[migrate] ✅ ${name}`); ok++; }
      else {
        const err = await r.text();
        console.log(`[migrate] ⚠️  ${err.includes('already exists') ? 'exists' : 'error'}: ${name}`);
        skip++;
      }
    } catch(e) {
